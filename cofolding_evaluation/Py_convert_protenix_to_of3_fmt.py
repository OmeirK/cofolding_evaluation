...
parser.add_argument('--protenix_dir', '-pd', help='Protenix results directory')
parser.add_argument('--out_dir', '-od', help='Output directory with protenix results reformatted')
...
def main():
    case_list = os.listdir(args.protenix_dir)
    ...
    for case in case_list:
        seed_list = os.listdir(f'{args.protenix_dir}/{case}/')
        
        for seed in seed_list:
            print(case, seed)
            case_outdir = f'{args.out_dir}/{case}/{seed}/'
            os.makedirs(case_outdir, exist_ok=True)
            output_files = glob.glob(f'{args.protenix_dir}/{case}/{seed}/predictions/*')
